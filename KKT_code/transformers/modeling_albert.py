...
@add_start_docstrings("Bert Model with a `language modeling` head on top.", ALBERT_START_DOCSTRING,
...
def split_context_query(sequence_output, pq_end_pos):
    context_max_len = sequence_output.size(1)  # 3b*max_seq_len*dim
    query_max_len = sequence_output.size(1)  # 3b*max_seq_len*dim
    sep_tok_len = 1  # [SEP]
    ...
    for i in range(0, sequence_output.size(0)):
        p_end = pq_end_pos[i][0]  # 上下文最后一位所在处
        q_end = pq_end_pos[i][1]  # QA最后一位所在处
...
def split_context_query_all(sequence_output, sep_pos):
    f_batch, max_seq_len, dim = sequence_output.size()
    context_sequence_output = sequence_output.new_zeros(f_batch, max_seq_len, dim)
    query_sequence_output = sequence_output.new_zeros(f_batch, max_seq_len, dim)
    query_attention_mask = sep_pos.new_zeros(f_batch, max_seq_len)
    context_attention_mask = sep_pos.new_zeros(f_batch, max_seq_len)
    for bid in range(f_batch):
        sep1 = sep_pos[bid][0]
        sep2 = sep_pos[bid][1]
        context_sequence_output[bid][:sep1 - 1] = sequence_output[bid][1:sep1]
        query_sequence_output[bid][:sep2 - sep1 - 1] = sequence_output[bid][sep1 + 1:sep2]
        context_attention_mask[bid][:sep1 - 1] = 1
        query_sequence_output[bid][:sep2 - sep1 - 1] = 1
    return context_sequence_output, query_sequence_output, context_attention_mask, query_attention_mask


def get_knowledge(knowledge_table, flat_knowledge_ids, toolman):
    f_batch, max_seq_len, dim = toolman.size()
    flat_knowldege_output = toolman.new_zeros(f_batch, max_seq_len, dim)
    for bid in range(f_batch):
        for (i, kid) in enumerate(flat_knowledge_ids[bid]):
            if kid == -1:
                break
            flat_knowldege_output[bid][i] = knowledge_table[kid]
    return flat_knowldege_output


def get_pivots(flat_sequence_output, flat_pivots_mask):
    f_batch, max_seq_len, dim = flat_sequence_output.size()
    flat_pivots_output = flat_sequence_output.new_zeros(f_batch, max_seq_len, dim)
    for bid in range(f_batch):
        for i in range(max_seq_len):
            if flat_pivots_mask[bid][i] == 1:
                flat_pivots_output[bid][i] = flat_sequence_output[bid][i]
    return flat_pivots_output
...
class AlbertForMultipleChoiceBaseline(AlbertPreTrainedModel):

    def __init__(self, config):
        super(AlbertForMultipleChoiceBaseline, self).__init__(config)
...
class AlbertForDialogueQA_all(AlbertPreTrainedModel):

    def __init__(self, config, knowledge_table, mode):
        super(AlbertForDialogueQA_all, self).__init__(config, knowledge_table, mode)
        self.albert = AlbertModel(config)
        ...
        self.from4to2 = nn.Linear(4 * config.hidden_size, 2 * config.hidden_size)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.att = BertCoAttention(config)
        self.init_weights()

    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None, labels=None,
                t_pq_end_pos=None, pivots_mask=None, qa_knowledge_ids=None, qa_knowledge_mask=None,
                contexts_knowledge_ids=None, contexts_knowledge_mask=None):

        b, op, max_seq_len = input_ids.size()

        flat_input_ids = input_ids.view(b * op, max_seq_len)
        flat_token_type_ids = token_type_ids.view(b * op, max_seq_len)
        flat_attention_mask = attention_mask.view(b * op, max_seq_len)
        flat_t_pq_end_pos = t_pq_end_pos.view(b * op, 2)
        flat_context_knowledge_ids = contexts_knowledge_ids.view(b * op, max_seq_len)
        flat_context_knowledge_mask = contexts_knowledge_mask.view(b * op, max_seq_len)
        flat_qa_knowledge_ids = qa_knowledge_ids.view(b * op, max_seq_len)
        flat_qa_knowledge_mask = qa_knowledge_mask.view(b * op, max_seq_len)
        flat_pivots_mask = pivots_mask.view(b * op, max_seq_len)
        ...
        flat_sequence_output = outputs[0]

        flat_context_output, flat_qa_output, flat_context_mask, flat_qa_mask = split_context_query(flat_sequence_output,
                                                                                                   flat_t_pq_end_pos)
        ...
        k_output = None
        pooled_output = None
        # the following part is duma origin
        MHA1_o = self.att(flat_context_output, flat_qa_output, flat_context_mask)
        MHA2_o = self.att(flat_qa_output, flat_context_output, flat_qa_mask)
        o_output = torch.cat([torch.mean(MHA2_o, 1), torch.mean(MHA1_o, 1)], 1)

        if 'p' in self.mode:
            # pivot_refined duma
            # flat_pivots_output=get_pivots(flat_sequence_output,flat_pivots_mask)
            p_context = self.att(flat_sequence_output, flat_context_output, flat_pivots_mask)
            MHA1_p = self.att(p_context, flat_qa_output, flat_context_mask)
            MHA2_p = self.att(flat_qa_output, p_context, flat_qa_mask)
            p_output = torch.cat([torch.mean(MHA2_p, 1), torch.mean(MHA1_p, 1)], 1)

        if 'k' in self.mode:
            # knowledge_refined duma
            flat_context_knowledge_output = get_knowledge(self.knowledge_table, flat_context_knowledge_ids,
                                                          flat_sequence_output)
            flat_qa_knowledge_output = get_knowledge(self.knowledge_table, flat_qa_knowledge_ids, flat_sequence_output)
            k_context = self.att(flat_context_knowledge_output, flat_context_output, flat_context_knowledge_mask)
            k_qa = self.att(flat_qa_knowledge_output, flat_qa_output, flat_qa_knowledge_mask)
            MHA1_k = self.att(k_context, k_qa, flat_context_mask)
            MHA2_k = self.att(k_qa, k_context, flat_qa_mask)
            k_output = torch.cat([torch.mean(MHA2_k, 1), torch.mean(MHA1_k, 1)], 1)

        if self.mode == 'kp':
            kp_combined = torch.cat([p_output, k_output], 1)
            # kp_output=gelu(self.from4to2(kp_combined))
            kp_output = self.from4to2(kp_combined)
            pooled_output = torch.cat([o_output, kp_output], 1)
            pooled_output = self.dropout(pooled_output)
            logits = self.classifier_2(pooled_output)
        elif self.mode == 'p':
            pooled_output = torch.cat([o_output, p_output], 1)
            pooled_output = self.dropout(pooled_output)
            logits = self.classifier_2(pooled_output)
        elif self.mode == 'k':
            pooled_output = torch.cat([o_output, k_output], 1)
            pooled_output = self.dropout(pooled_output)
            logits = self.classifier_2(pooled_output)
        elif self.mode == 'o':
            pooled_output = o_output
            pooled_output = self.dropout(pooled_output)
            logits = self.classifier(pooled_output)
        else:
            raise ValueError('mode not found in [kp,k,p,o]')

        reshaped_logits = logits.view(-1, op)
...
class AlbertForDialogueQA_pivots(AlbertPreTrainedModel):

    def __init__(self, config, knowledge_table, mode):
        super(AlbertForDialogueQA_pivots, self).__init__(config, knowledge_table, mode)
        self.albert = AlbertModel(config)
        self.knowledge_table = knowledge_table
        self.mode = mode
        self.classifier = nn.Linear(2 * config.hidden_size, 1)
...
class AlbertForKnowledgeRepresentation(AlbertPreTrainedModel):
    def __init__(self, config):
        super(AlbertForKnowledgeRepresentation, self).__init__(config)
        self.albert = AlbertModel(config)
        self.init_weights()
        self.SAttention = BertCoAttention(config)
    ...
    def forward(self, input_ids=None, attention_mask=None):
        ...
        sequence_output = outputs[0]  # batch*max_seq_len*hiddensize
        refined_seq_output = self.SAttention(sequence_output, sequence_output, attention_mask)
        knowledge_per_batch = None
        for bid in range(input_ids.size()[0]):
            seq = refined_seq_output[bid]
            hid = attention_mask[bid]
            useful_length = hid.sum().item()
            useful_output = torch.mean(seq[1:useful_length], dim=0).unsqueeze(0)
            if knowledge_per_batch is None:
                knowledge_per_batch = useful_output
            else:
                knowledge_per_batch = torch.cat([knowledge_per_batch, useful_output], dim=0)
        return knowledge_per_batch
